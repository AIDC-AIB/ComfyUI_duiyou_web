...
import { ComfyUIClient } from "./comfySdk.js";
import { nanoid } from './utils.js';
import { cookieManager } from './cookie.js';
import { ComfySocketMessage } from './socketMessage.js';

export const ANIM_PREVIEW_WIDGET = "$$comfy_animation_preview"
const ENABLE_WORKFLOW_CACHE_FEATURE = false;
...
export class ComfyApp {
	...
	constructor() {
		...
		this.comfySdk = new ComfyUIClient(this);
    this.comfySocketMessage = new ComfySocketMessage(this);
					...
					if (this.images !== output.images) {
						...
						imgURLs = imgURLs.concat(
							output.images.map((params) => {
								return api.apiURL(
									"/view?" +
										new URLSearchParams({...params, userToken: api.userToken , userId: api.userInfo.userId }).toString() +
	...
	#addApiUpdateHandlers() {
		...
		api.addEventListener("execution_error", ({ detail }) => {
			console.log('===>> [app.js] execution_error', detail);
	...
	async #loadExtensions() {
        ...
        // 过滤掉 checkVersion 相关脚本文件
        .filter((ext) => !/checkVersion/i.test(ext))
        .map(async ext => {
            try {
	...
	async setup() {
    cookieManager.setup();
		await this.comfySdk.setup();
		this.#addApiUpdateHandlers();
		await Promise.all([this.#setUser(), this.ui.settings.load(), this.#loadExtensions()])
		...
		let restored = false;
		if (ENABLE_WORKFLOW_CACHE_FEATURE) {
			try {
	...
	/**
	 * Validates the current graph workflow
	 *
	 * @returns {Promise<import('/types/app.js').ValidateError | { prompt_id: string, nodeIds: string[] }>}
	 */
	async validatePrompt() {
		try {
			// 此处需要小心
			// 之所以复用api.queuePrompt是因为第三方插件和节点会对api.queuePrompt的入参做修改，同时会把我们自定义传入的参数弄丢
			// 所以我们要想把flow_id传过去，只能将其挂到api上 => `api.currentFlowId`
			// 当执行结束后，在finally中将其置空
			api.currentFlowId = this.comfySdk.currentWorkflow?.id;
			// 没有flow_id，则生成一个
			if (!api.currentFlowId) {
				api.currentFlowId = nanoid();
			}

      // api.queuePrompt 被改造为仅作为一个校验的功能
      // 因为第三方插件和节点会对api.queuePrompt做修改，同时
      const { output, workflow } = await this.graphToPrompt();
      if (Object.keys(output).length === 0) {
        throw new Error("No nodes in the graph!");
      }

			return await api.queuePrompt(undefined, { output, workflow });
		} catch (error) {
			...
			/** @type {import('/types/app.js').ValidateError} */
			let validateError;
			if (error.response && error.response.node_errors) {
				validateError = {
					message: formattedError,
					node_errors: error.response.node_errors,
				}
			} else {
				validateError = {
      ...
      // 主动清理 api.currentFlowId
      api.currentFlowId = null;
    }
	}

	async queuePrompt(number, batchCount = 1) {
		// this.comfySdk.currentPromptId 如果为空，说明没有对工作流进行校验，不允许执行
		if (!this.comfySdk.currentPromptId) {
			this.ui.dialog.show("Please validate the workflow first!");
			return;
		}
		...
		try {
			while (this.#queueItems.length) {
				...
				for (let i = 0; i < batchCount; i++) {
					...
					try {
						const res = await api.executePrompt(this.comfySdk.currentPromptId);
						this.lastNodeErrors = res.node_errors;
						if (this.lastNodeErrors && this.lastNodeErrors.length > 0) {
							this.canvas.draw(true, true);
						}
					} catch (error) {
						const formattedError = this.#formatPromptError(error)
	...
	registerExtension(extension) {
		...
		// 禁用某些不需要的组件
		if (['mtb.ImageFeed', 'pysssss.ImageFeed'].includes(extension.name)) {
			return;
		}
    ...
    this.comfySocketMessage.clearState();
	}
}
