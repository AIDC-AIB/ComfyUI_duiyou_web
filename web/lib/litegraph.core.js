...
*/
      ...
      if (info.widgets_values) {
        for (var i = 0; i < info.widgets_values.length; ++i) {
          if (this.widgets[i]) {
            ...
            // 对存在 `_widgets_values_map` 的节点数据特殊处理
            // 补全 widget 对象属性 `serializeValue` 和 `serializeValueSync`
            if (
              info._widgets_values_map &&
              info._widgets_values_map[widget.value]
            ) {
              const widgetOriginData = info._widgets_values_map[widget.value];
              if (widgetOriginData) {
                widget.value = widgetOriginData.name;
                // window.comfyUtil.serializeWidgetValue 在 /web/scripts/utils.js 中定义的
                widget.serializeValue =
                  window.comfyUtil.serializeWidgetValue(widgetOriginData);
                widget.serializeValueSync = widget.serializeValue;
                this._widgets_values_map[widget.value] = widgetOriginData;
              }
            }
  ...
  LGraphNode.prototype.serialize = function () {
    ...
    if (this.widgets && this.serialize_widgets) {
      o.widgets_values = [];
      o._widgets_values_map = {};
      for (var i = 0; i < this.widgets.length; ++i) {
        if (this.widgets[i]) {
          const widget = this.widgets[i];

          let widgetValue = widget.value;
          // widget.serializeValueSync 是在 comfySdk.js 中定义的
          // 仅在 底模 和 Lora 切换时会定义
          if (widget.serializeValueSync) {
            widgetValue = widget.serializeValueSync();
          }

          o.widgets_values[i] = widgetValue;
          if (
            this._widgets_values_map &&
            this._widgets_values_map[widget.value]
          ) {
            o._widgets_values_map[widgetValue] =
              this._widgets_values_map[widget.value];
          }
        } else {
  ...
  LGraphNode.prototype.connect = function (slot, target_node, target_slot) {
    ...
    if (output.links && output.links !== null && output.links.length) {
      switch (output.type) {
    ...
    //connect in output
    if (output.links == null || output.links === undefined) {
      output.links = [];
    }
...
*/
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            node.inResizeCorner(e.canvasX, e.canvasY)
          ) {
            this.graph.beforeChange();
        ...
        if (!skip_action) {
          ...
          if (widget) {
            ...
            // intercept widget click event
            if (
              window.comfySdk &&
              window.comfySdk.processNodeWidgetClickHandler
            ) {
              window.comfySdk.processNodeWidgetClickHandler(widget, node);
            }
          }
  ...
  LGraphCanvas.prototype.drawButton = function (
    x,
    y,
    w,
    h,
    ...
    var clicked =
      pos && LiteGraph.isInsideRectangle(pos[0], pos[1], x, y, w, h);
  ...
  LGraphCanvas.prototype.drawConnections = function (ctx) {
    ...
    for (var n = 0, l = nodes.length; n < l; ++n) {
      ...
      for (var i = 0; i < node.inputs.length; ++i) {
        ...
        var end_dir =
          end_slot.dir || (node.horizontal ? LiteGraph.UP : LiteGraph.LEFT);
  ...
  LGraphCanvas.onMenuAdd = function (node, options, e, prev_menu, callback) {
    ...
    function inner_onMenuAdded(base_category, prev_menu) {
      ...
      var nodes = LiteGraph.getNodeTypesInCategory(
  ...
  LGraphCanvas.prototype.showLinkMenu = function (link, e) {
    ...
    if (
      node_right &&
      node_right.outputs &&
      node_right.outputs[link.target_slot]
    )
  ...
  LGraphCanvas.prototype.showConnectionMenu = function (optPass) {
    ...
    function inner_clicked(v, options, e) {
      ...
      switch (v) {
        ...
        default:
          // check for defaults nodes for this slottype
          var nodeCreated = that.createDefaultNodeForSlot(
            Object.assign(opts, {
  ...
  LGraphCanvas.prototype.createPanel = function (title, options) {
    ...
    root.addWidget = function (type, name, value, options, callback) {
      ...
      if (type == 'code')
        ...
        var str_value = LGraphCanvas.getPropertyPrintableValue(
          value,
          options.values,
        );
        value_element.innerText = str_value;
  ...
  LGraphCanvas.prototype.showShowGraphOptionsPanel = function (
    refOpts,
    obEv,
    refMenu,
    refMenu2,
    ...
    if (this.constructor && this.constructor.name == 'HTMLDivElement') {
      ...
      if (
        ...
        !obEv.event ||
        !obEv.event.target ||
        !obEv.event.target.lgraphcanvas
      ) {
    ...
    function inner_refresh() {
      ...
      for (var pName in node.properties) {
        ...
        if (
          ...
          node.onAddPropertyToPanel(pName, panel)
        )
          continue;
  ...
  LiteGraph.pointerListenerAdd = function (
    oDOM,
    sEvIn,
    fCall,
    capture = false,
